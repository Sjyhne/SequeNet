...
class ABL(nn.Module):
    ...
    def logits2boundary(self, logit):
        ...
        while True: # avoid the case that full image is the same color
            ...
            if kl_combine_bin.sum() > max_N:
                ...
                break
        #dilate
        dilate_weight = torch.ones((1,1,3,3))
    ...
    def get_direction_gt_predkl(self, pred_dist_map, pred_bound, logits):
        ...
        dist_maps = torch.zeros((0,len(x))) # 8k
        kl_maps = torch.zeros((0,len(x))) # 8k
    ...
    def forward(self, logits, target):
        ...
        print(direction_gt.shape, direction_pred.shape, weight_ce.shape)
        print(direction_gt)
