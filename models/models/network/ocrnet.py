...
from .utils import get_trunk, get_trunk_v2
...
class MScale(nn.Module):
    ...
    OCR net
    """
    def __init__(self, num_classes, num_channels, trunk='hrnetv2', criterion=None):
        super(MScale, self).__init__()
        self.criterion = criterion
        self.backbone, _, _, high_level_ch = get_trunk(trunk, num_channels)
        self.ocr = OCR_block(high_level_ch)
...
def HRNet_Mscale(num_classes, num_channels, criterion):
    model =  MScale(num_classes, num_channels, trunk='hrnetv2', criterion=criterion)
    #model.backbone.init_weights(cfg.ASSETS_PATH)
    return model

class MScaleV2(nn.Module):
    """
    OCR net
    """
    def __init__(self, num_classes, trunk='hrnetv2', criterion=None):
        super(MScaleV2, self).__init__()
        self.criterion = criterion
        self.backbone_3, self.backbone_1, _, _, high_level_ch = get_trunk_v2(trunk)
        self.ocr = OCR_block(high_level_ch)
        self.scale_attn = make_attn_head(
            in_ch=cfg.MODEL.OCR.MID_CHANNELS, out_ch=1)
    ...
    def _fwd(self, x):
        ...
        _, _, high_level_features_3 = self.backbone_3(x_3)
        _, _, high_level_features_1 = self.backbone_1(x_1)
        
        #high_level_features = torch.cat((high_level_features_3, high_level_features_1), dim=1)
        high_level_features = torch.add(high_level_features_3, high_level_features_1)
...
def HRNet_Mscale_v2(num_classes, criterion):
    model =  MScaleV2(num_classes, trunk='hrnetv2', criterion=criterion)
    #model.backbone.init_weights(cfg.ASSETS_PATH)
    return model