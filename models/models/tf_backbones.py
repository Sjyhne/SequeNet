...
def create_base_model(name="ResNet50", weights="imagenet", height=None, width=None,
    ...
    if name.lower() == "densenet121":
        ...
        if height <= 31 or width <= 31:
            raise ValueError("Parameters 'height' and 'width' should not be smaller than 32.")
        base_model = tf.keras.applications.MobileNetV2(include_top=False, weights=weights, input_shape=input_shape, pooling=pooling, alpha=alpha)
        ...
        if height <= 31 or width <= 31:
            raise ValueError("Parameters 'height' and 'width' should not be smaller than 32.")
        base_model = tf.keras.applications.NASNetLarge(include_top=False, weights=weights, input_shape=input_shape, pooling=pooling)
