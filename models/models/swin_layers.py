...
class Mlp(tf.keras.layers.Layer):
    def __init__(self, filter_num, drop=0., name=''):
        ...
        # MLP layers
        self.fc1 = Dense(filter_num[0], name='{}mlp0'.format(name))
        self.fc2 = Dense(filter_num[1], name='{}mlp1'.format(name))
...
class WindowAttention(tf.keras.layers.Layer):
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0, proj_drop=0., name=''):
        ...
        # Layers
        self.qkv = Dense(dim * 3, use_bias=qkv_bias, name='{}attnqkv'.format(self.prefix))
        self.attn_drop = Dropout(attn_drop)
        self.proj = Dense(dim, name='{}attnproj'.format(self.prefix))
    ...
    def build(self, input_shape):
        ...
        self.relative_position_bias_table = self.add_weight('{}attnpos'.format(self.prefix),
        ...
        self.relative_position_index = tf.Variable(
            initial_value=tf.convert_to_tensor(relative_position_index), trainable=False, name='{}attnposind'.format(self.prefix))
...
class SwinTransformerBlock(tf.keras.layers.Layer):
    def __init__(self, dim, num_patch, num_heads, window_size=7, shift_size=0, num_mlp=1024,
        ...
        # Layers
        self.norm1 = LayerNormalization(epsilon=1e-5, name='{}norm1'.format(self.prefix))
        ...
        self.norm2 = LayerNormalization(epsilon=1e-5, name='{}norm2'.format(self.prefix))
    ...
    def build(self, input_shape):
        if self.shift_size > 0:
            ...
            self.attn_mask = tf.Variable(initial_value=attn_mask, trainable=False, name='{}attnmask'.format(self.prefix))
        else:
            self.attn_mask = None
