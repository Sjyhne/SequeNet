...
class HRNet(tf.keras.models.Model):
    ...
    def __init__(self, height=512, width=512, channel=3, classes=2):
        ...
        self.out_filters_list = [16, 32, 64, 128]
        
        self.StemNet = StemNet((None, height, width, channel))
        
        self.TransitionLayer1 = TransitionLayer1(self.out_filters_list[:2])
        self.TransitionLayer2 = TransitionLayer2(self.out_filters_list[:3])
        self.TransitionLayer3 = TransitionLayer3(self.out_filters_list[:4])
        ...
        self.Branch1_0 = Branch(16)
        self.Branch1_1 = Branch(32)

        self.Branch2_0 = Branch(16)
        self.Branch2_1 = Branch(32)
        self.Branch2_2 = Branch(64)

        self.Branch3_0 = Branch(16)
        self.Branch3_1 = Branch(32)
        self.Branch3_2 = Branch(64)
        self.Branch3_3 = Branch(128)
        
        self.AuxHead = tf.keras.Sequential([
            ConvolutionBnActivation(360, (1, 1)),
            tf.keras.layers.Conv2D(filters=classes, kernel_size=(1, 1), use_bias=True),
            tf.keras.layers.Activation("softmax")
            ])
        
        self.Conv3x3BnReluOcr = ConvolutionBnActivation(256, (3, 3))
        
        self.SpatialContext = SpatialGather_Module(scale=1)
        self.SpatialOcr = SpatialOCR_Module(256, scale=1, dropout=0.05)
...
def hrnetocr(n_classes, input_height=None, input_width=None):
    model = HRNet()
