...
class patch_merging(tf.keras.layers.Layer):
    ...
    def __init__(self, num_patch, embed_dim, name=''):
        ...
        self.linear_trans = Dense(2*embed_dim, use_bias=False, name='{}lineartrans'.format(name))
...
class patch_expanding(tf.keras.layers.Layer):
    ...
    def __init__(self, num_patch, embed_dim, upsample_rate, return_vector=True, name=''):
        ...
        self.linear_trans1 = Conv2D(upsample_rate*embed_dim, kernel_size=1, use_bias=False, name='{}lineartrans1'.format(name))
        # 
        self.linear_trans2 = Conv2D(upsample_rate*embed_dim, kernel_size=1, use_bias=False, name='{}lineartrans1'.format(name))
        self.prefix = name
    ...
    def call(self, x):
        ...
        x = tf.nn.depth_to_space(x, self.upsample_rate, data_format='NHWC', name='{}dtospace'.format(self.prefix))
